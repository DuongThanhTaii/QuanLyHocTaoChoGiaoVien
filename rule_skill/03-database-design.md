## 1. Tai sao chon Supabase?

| Tieu chi | Supabase | Neon.tech | Firebase |
|----------|----------|-----------|----------|
| **Relational** | Co PostgreSQL | Co PostgreSQL | Khong NoSQL |
| **Auth built-in** | Co Multi-provider | Khong Tu build | Co Firebase Auth |
| **Realtime** | Co Native subscriptions | Khong Can them | Co Native |
| **Storage** | Co Built-in | Khong Tich hop them | Co Built-in |
| **RLS (Security)** | Co Row Level Security | Can Manual | Co Rules |
| **Edge Functions** | Co Deno/TS | Khong | Khong |
| **Self-hostable** | Co | Co | Khong |
| **Pricing** | Co Generous free tier | Co | Co |

**Quyet dinh:** Dung **Supabase** lam primary platform. Neon.tech chi can nhac neu sau nay can read-replicas cho analytics heavy.

## 2. Schema Design (PostgreSQL)

### 2.1 Identity & Access Management
```sql
-- Bang users mo rong tu auth.users cua Supabase
...
-- Lien ket phu huynh - hoc sinh (n-n)
...
-- Thoi khoa bieu (ho tro recurring)
...
### 2.6 Subscription (Admin billing)
```sql
...
-- Giao vien chi xem hoa don cua minh
CREATE POLICY "Teachers view own invoices" ON public.invoices
  FOR ALL USING (auth.uid() = teacher_id);

-- Phu huynh/Hoc sinh chi xem hoa don lien quan den minh
CREATE POLICY "Students view own invoices" ON public.invoices
  FOR SELECT USING (auth.uid() = student_id);

CREATE POLICY "Parents view child invoices" ON public.invoices
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.parent_students 
      WHERE parent_id = auth.uid() AND student_id = invoices.student_id
    )
  );
...
```typescript
// Lang nghe diem danh real-time
supabase
  .channel('attendance-class-123')
  .on('postgres_changes', 
    { event: '*', schema: 'public', table: 'attendance_records', filter: 'class_id=eq.123' },
    (payload) => { /* Update UI */ }
  )
  .subscribe();
```
