            ...
            <Card className="border-0 shadow-lg bg-white rounded-3xl overflow-hidden">
              <CardContent className="p-5 sm:p-8 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {(['present', 'late', 'absent', 'excused'] as const).map((status) => <div key={status} className="rounded-xl border border-blue-100 bg-blue-50/70 p-3 text-center"><p className="text-lg font-bold text-blue-700">{learningReport.sessions.filter((session: any) => session.attendanceStatus === status).length}</p><p className="text-xs text-zinc-600">{attendanceLabel[status]}</p></div>)}
                </div>
