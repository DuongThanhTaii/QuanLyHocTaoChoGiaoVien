        ...
        <div className="p-6 md:p-8 space-y-6 bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 print:p-0">
          ...
          {viewTab === 'report' && (
            learningReport ? <div className="rounded-2xl border border-zinc-200 bg-white p-5 sm:p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{(['present', 'late', 'absent', 'excused'] as const).map((status) => <div key={status} className="rounded-xl border border-blue-100 bg-blue-50/70 p-3 text-center"><p className="text-lg font-bold text-blue-700">{learningReport.sessions.filter((session: any) => session.attendanceStatus === status).length}</p><p className="text-xs text-zinc-600">{attendanceLabel[status]}</p></div>)}</div>
