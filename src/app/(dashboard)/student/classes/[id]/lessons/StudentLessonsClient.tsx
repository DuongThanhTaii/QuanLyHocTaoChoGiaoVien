...
export function StudentLessonsClient({ classId, lessons, exercises, sessions, mySubmissions }: Props) {
  ...
  return <div className="space-y-6">
    <section className="border-b border-zinc-200 pb-4 dark:border-zinc-800">
      ...
      <div className="mt-1 flex items-center gap-2"><CalendarDays className="h-5 w-5 text-blue-600" /><h2 className="text-xl font-semibold tracking-tight text-zinc-950 dark:text-zinc-50">{selected ? dateName(selected.session_date) : 'Chưa có lịch học'}</h2></div>
      {selected && <p className="mt-1 text-sm text-zinc-500">{time(selected) ? `Buổi học ${time(selected)}` : 'Buổi học đã lên lịch'}{selected.session_date === today ? ' · Hôm nay' : isFuture ? ' · Sắp diễn ra' : ''}</p>}
