...
const backgroundIcons = [
  ...
  { Icon: Atom, className: "left-[38%] top-[8%] size-14 rotate-12" }, { Icon: BookOpen, className: "right-[5%] top-[22%] size-16 rotate-12" },
  { Icon: PencilLine, className: "right-[20%] top-[8%] size-12 rotate-35" }, { Icon: Calculator, className: "right-[8%] bottom-[17%] size-16 rotate-12" },
  { Icon: BookOpen, className: "left-[8%] bottom-[15%] size-18 -rotate-12" }, { Icon: Atom, className: "right-[25%] bottom-[7%] size-14 -rotate-12" },
];
...
function PasswordField({ id, label, show, onToggle }: { id: string; label: string; show: boolean; onToggle: () => void }) {
  return <div><label className="block text-xs font-semibold text-zinc-600" htmlFor={id}>{label}</label><div className="relative mt-1.5"><input id={id} name={id} type={show ? "text" : "password"} required autoComplete="new-password" className="h-10 w-full rounded-xl border border-[#e9c999] bg-[#fffaf2] px-3 pr-10 text-sm text-zinc-800 outline-none transition focus:border-[#ed8d35] focus:ring-3 focus:ring-[#f7bd76]/35 [&::-ms-reveal]:hidden [&::-ms-clear]:hidden" /><button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#ef7616] transition hover:text-[#c95508]" aria-label={show ? "Ẩn mật khẩu" : "Hiện mật khẩu"}>{show ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div></div>;
}
